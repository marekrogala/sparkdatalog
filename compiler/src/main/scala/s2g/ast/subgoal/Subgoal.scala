...
trait Subgoal {
  def join(valuations: RDD[Valuation], boundVariables: Set[String], database: Database): RDD[Valuation]
  def select(initialValuations: Set[Valuation], boundVariables: Set[String], database: Database): RDD[Valuation]
