...
object ExtendWithSwapRelations {
  def apply(program: Program): Program = {
    val newRules =
      for((rule, ruleId) <- program.rules.zipWithIndex;
          (ruleBody, bodyId) <- rule.ruleBodies.zipWithIndex) yield {
          val subgoals = ruleBody.dynamicSubgoals.map(_._1)
          val constantSubgoals: Seq[Subgoal] = ruleBody.constantSubgoals.map(_._1)
          ...
          if(subgoals.size > 0) {
            ...
            val (predicates, auxRules) = subsequencesWithoutLastWithDest.zipWithIndex.map({
              case ((subgoalsSeq, dest), auxRuleId) =>
                val auxRuleName = "_aux_%s_%d_%d_%d_".format(rule.head.name, ruleId, bodyId, auxRuleId)
                val auxRuleBody = RuleBody(subgoalsSeq)
                val auxRuleArgs = dest +: auxRuleBody.outVariables.toSeq
                (Predicate(auxRuleName, auxRuleArgs.map(ValueVar)), Rule(Head(auxRuleName, auxRuleArgs), Seq(auxRuleBody)))
            }).unzip
            val mainRuleBody = RuleBody(predicates.map(GoalPredicate) ++ lastSubsequence)
            val mainRule = Rule(rule.head, Seq(mainRuleBody))
            val newRulesForRule = mainRule +: auxRules
            newRulesForRule
          } else {
            val newRulesForRule = Seq(Rule(rule.head, Seq(ruleBody)))
            newRulesForRule
          }
        }
